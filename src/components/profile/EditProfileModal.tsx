...
// Dossier editor — bio, base location, website, banner, and social
// links. Values persist to users.metadata via PATCH /api/user/profile;
// the server sanitizes everything (lengths, URL schemes, social hosts),
// so this form stays honest about limits but never blocks on them.
      ...
      >
        <div className="flex items-center justify-between border-b border-white/[0.08] px-5 py-3">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[10px] tracking-[0.4em] text-zinc-300">EDIT DOSSIER</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
          <Field label="MISSION STATEMENT" hint={`${form.bio.length}/${BIO_MAX}`}>
            <textarea
              value={form.bio}
              onChange={(e) => set({ bio: e.target.value.slice(0, BIO_MAX) })}
              rows={3}
              placeholder="Who are you out here? What are you building?"
