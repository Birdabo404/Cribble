...
// in the same visual family as the app's dither-kit surfaces. Output
...
void main() {
  ...
  // Heat radiates from the avatar: gaussian falloff across the width
  // over a small floor so the far side never reads fully dead, then a
  // vertical seat so the glow sits inside the banner rather than
  // slicing across its edges.
  float dx = abs(st.x - uOrigin);
  float radiate = mix(0.18, 1.0, exp(-5.5 * dx * dx));
  ...
  float intensity = (0.035 + 0.13 * caustic) * radiate * seat * uHeat;
  ...
  // Gold body, whitened right at the wavefront.
  vec3 color = mix(uGold, vec3(1.0), 0.55 * crest * wave);
