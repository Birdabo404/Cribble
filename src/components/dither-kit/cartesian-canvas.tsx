...
// [Cribble patch] React 18: RefObject<T>.current is T | null under strict null
// checks — the loop needs the mutable, non-null flavour, so MutableRefObject.
...
type LoopArgs = {
  ...
  cols: number
  rows: number
  state: MutableRefObject<ChartContextValue>
  targets: MutableRefObject<Record<string, Surface>>
  stars: MutableRefObject<Star[]>
  // [Cribble patch] Reduced motion is passed in live (from useReducedMotionLive)
  // instead of read once via prefersReducedMotion(), so the Appearance toggle
  // flipping html[data-motion] restarts the loop in the right mode.
  reduce: boolean
}
...
function startCartesianLoop({
  ...
  // [Cribble patch] `reduce` now arrives as a LoopArgs field (see above).
...
export function CartesianCanvas() {
  ...
  // [Cribble patch] Live reduced-motion (OS media query + html[data-motion]).
  ...
  useEffect(() => {
    ...
    return startCartesianLoop({
      ...
      targets: targetsRef,
      stars: starsRef,
      // [Cribble patch] Restart the loop when reduced motion flips at runtime.
