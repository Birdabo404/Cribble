// [Cribble patch] react imports back useReducedMotionLive (see bottom of file).
...
/** Whether reduced motion is requested (snap + steady stars).
 * [Cribble patch] Honors BOTH the OS media query and Cribble's in-app
 * Appearance toggle, which navBoot mirrors onto <html data-motion="reduced">. */
export function prefersReducedMotion() {
  return (
    (window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches ??
      false) || document.documentElement.dataset.motion === "reduced"
  )
}

/** [Cribble patch] Live reduced-motion state. Both sources flip at runtime
 * (the Appearance toggle rewrites html[data-motion] without a reload), so the
 * canvases subscribe — media-query change event + a MutationObserver on the
 * attribute — and restart their paint loops when the value changes. */
export function useReducedMotionLive(): boolean {
  const [reduced, setReduced] = useState(false)
  useEffect(() => {
    const media = window.matchMedia("(prefers-reduced-motion: reduce)")
    const compute = () => setReduced(prefersReducedMotion())
    compute()
