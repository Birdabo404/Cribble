...
import { datafastStatsUrl } from '@/lib/datafastPublic'

const POLL_MS = 30_000
const STATS_URL = datafastStatsUrl()
...
export function VisitorTicker() {
  ...
  const chip =
    'inline-flex max-w-full flex-wrap items-center justify-center gap-x-2 gap-y-1 rounded-full border border-[rgb(var(--lb-panel-edge)/0.1)] bg-[rgb(var(--lb-panel-edge)/0.04)] px-3 py-1.5 font-data text-[10px] tracking-[0.08em] text-zinc-500 sm:gap-x-2.5 sm:px-3.5 sm:tracking-[0.12em]'
    ...
    <div className="mt-5 flex justify-center px-1">
      {STATS_URL ? (
        <a
          ...
          aria-label={`${countsLabel}. Open Cribble stats on DataFast`}
          className={`${chip} transition-colors hover:border-[rgb(var(--lb-panel-edge)/0.2)] hover:bg-[rgb(var(--lb-panel-edge)/0.07)]`}
        >
          {body}
        </a>
      ) : (
        <p role="status" aria-live="polite" aria-label={countsLabel} className={chip}>
          {body}
        </p>
      )}
    </div>
