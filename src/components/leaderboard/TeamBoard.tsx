...
// with the team score above.
const MEMBER_GRID =
  'grid grid-cols-[3.6rem_minmax(0,1fr)_auto_1rem] md:grid-cols-[4.2rem_minmax(0,1fr)_7rem_auto_1rem] items-center gap-3 px-4 md:px-5'
...
export function TeamBoard() {
  ...
  const jumpToMyTeam = useCallback(() => {
    ...
    requestAnimationFrame(() => {
      rowRefs.current.get(target.userId)?.scrollIntoView({
        block: 'center',
  ...
  return (
    <>
      ...
      <section className="lbt-reveal">
        <div className="lb-panel grid grid-cols-2 overflow-hidden rounded-2xl md:grid-cols-3">
          <StatCell
  ...
  return (
    <li>
      ...
      >
        ...
        >
          <span className="block h-0.5 overflow-hidden rounded-full bg-[rgb(var(--lb-panel-edge)/0.07)]">
            <span
...
function YourTeamBar({ team, onJump }: { team: TeamBoardRow; onJump: () => void }) {
  return (
    <button
      ...
      style={{
        background:
          'linear-gradient(180deg, rgb(var(--accent-rgb) / 0.07), rgb(var(--accent-rgb) / 0.03)), rgb(var(--lb-panel-bg) / 0.88)',
    ...
    >
      <div className="flex items-center gap-3 px-4 py-3 md:gap-4 md:px-5">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[9px] tracking-[0.35em] text-zinc-500">YOUR TEAM</div>
          <div className="mt-1 truncate text-[11px] text-zinc-300">@{team.username}</div>
        </div>

        <div className="shrink-0 text-right text-[15px] leading-none text-zinc-50 tabular-nums [font-family:var(--font-pixel)]">
          {formatNumber(team.score)}
        </div>
      </div>
    </button>
