// CDP verification harness for the Cherry Blossom plate rebuild. Spawns
// headless Brave on port 9237, forces the signed-out cosmetics state, finds
// the Cherry Blossom tile in the regular shop grid (epic — not the Reserve
// shelf) and captures: the full tile + preview (dark/light), a 1000×68
// leaderboard-row simulation (full strength + the 55% rest dim),
// deterministic animation-clock freezes at the choreographed moments
// (the gust, the updraft stall, the early flurry, the cloud crossing),
// and frozen reduced-motion frames with a pixel-stability check.
//
//   node scripts/blossom-plate-shots.mjs [label] [base-url]
//
// Choreography math: freezing every animation in the tile at one document
// ms pins each petal on its own clock — phase = (ms − delay) / duration.
...
const PORT = 9237
const OUT = new URL('./shots-blossom/', import.meta.url).pathname
...
async function main() {
  ...
  // Tag the Cherry Blossom tile + preview so every helper can address them
  // with plain selectors (the grid has no per-plate ids). Epic plate in the
  // REGULAR grid — assert it did not drift into the Reserve shelf.
  const tagBlossom = async () => {
    ...
    if (found === 'reserve') throw new Error('cherry-blossom must sell from the regular grid, found it on the Reserve shelf')
  }
   ...
   * (negative delays fold into each animation's phase). */
  ...
  const TILE = 'article[data-blossom]'
  const PREVIEW = '[data-blossom-preview]'

  // Choreography clock positions (document ms → keyframe phase):
  //   gust   near petal 1: 7.6s clock, delay −1.2s, gust 63%
  //          → (0.63·7600) − 1200 = 3588  (tumble at 65% → back face)
  //   stall  mid petal 1: 10.5s clock, delay −7.7s, updraft 52%
  //          → (0.52·10500) − 7700 + 10500 = 8260
  //   flurry near petal 3: 7.9s clock, delay −3.4s, mid-sway 73%
  //          → (0.73·7900) − 3400 = 2367 — the whole field awake
  //   cloud  cloud A: 72s clock, delay −18s, mid-crossing 50%
  //          → (0.50·72000) − 18000 = 18000
  const GUST_MS = 3588
  const STALL_MS = 8260
  const FLURRY_MS = 2367
  const CLOUD_MS = 18000
  ...
  await freezeAnimationsAt(TILE, GUST_MS)
  await clipShot('03-gust-freeze', PREVIEW, 6)
  await freezeAnimationsAt(TILE, STALL_MS)
  await clipShot('04-stall-freeze', PREVIEW, 6)
  await freezeAnimationsAt(TILE, FLURRY_MS)
  await clipShot('05-flurry-freeze', PREVIEW, 6)
  await freezeAnimationsAt(TILE, CLOUD_MS)
  await clipShot('06-cloud-freeze', PREVIEW, 6)
  ...
  await sleep(400)
  await scrollTo(TILE)
  await clipShot('07-row68-dark', PREVIEW, 6)
  await freezeAnimationsAt(TILE, GUST_MS)
  await clipShot('08-row68-gust', PREVIEW, 6)
  await thawAnimations(TILE)
  await evalJs(
    `document.querySelector('${PREVIEW}').firstElementChild.style.opacity = '0.55'; 'ok'`
  )
  await sleep(200)
  await clipShot('09-row68-dim55', PREVIEW, 6)
  await evalJs(
  ...
  await scrollTo(TILE)
  await clipShot('10-tile-light', TILE)
