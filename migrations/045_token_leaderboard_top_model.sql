-- Extend the service-only Burn Board aggregate with an honest primary model.
-- "Top" means the agent or model reported on the most distinct synced days
-- in the selected window. Ties prefer the most recent observation, then a
-- stable name sort. This replaces only the read function; no usage rows move.
...
as $$
  ...
  model_counts as (
    select
      filtered.user_id,
      lower(btrim(model_name)) as model_name,
      count(distinct filtered.date)::bigint as active_days,
      max(filtered.ingested_at) as last_seen_at
    from filtered
    cross join lateral unnest(filtered.models) as model_name
    where btrim(model_name) <> ''
    group by filtered.user_id, lower(btrim(model_name))
  ),
  ranked_models as (
    select
      model_counts.*,
  ...
  select
    ...
    coalesce(top_models.active_days, 0)::bigint as top_model_days
  from totals
...
comment on function public.agent_token_leaderboard(date, date) is
  'Service-only aggregate for opted-in active users, including the most frequently reported agent and model by distinct active day. Values are self-reported and estimated cost is not a billing receipt.';
