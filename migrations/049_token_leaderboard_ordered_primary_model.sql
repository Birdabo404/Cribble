-- ============================================================
-- Migration 049: Restore trusted primary models on the Burn Board
-- ============================================================
-- Agent v1.2+ orders each daily models array by ccusage token volume.
-- Migration 047 preserved the full legacy model mix, but its leader CTE
-- accepted only cardinality(models) = 1. A correctly synced Codex day that
-- used more than one model therefore returned a NULL top_model even though
-- the trusted daily primary was models[1].
--
-- Keep exact model breakdowns conservative: event rows and single-model
-- legacy rows remain the only facts that receive exact token attribution.
-- Ordered multi-model legacy rows participate only in primary-model ranking;
-- their whole-day tokens are never exposed as an exact model token share.
-- ============================================================
