-- ============================================================
-- Migration 057: Legacy fallback for the Burn Board top agent
-- ============================================================
-- 050 ranks top_agent from exact facts only: v2 events plus legacy days
-- that reported exactly one agent. Users whose attributable days all
-- reported multiple agents end up with top_agent = null, so the board
-- renders "Mixed" even though their history names a clear daily driver.
--
-- Mirror the model_rank_facts approach from 050: the exact token-weighted
-- rank stays first priority, and only when no exact rank-1 agent exists we
-- fall back to a presence rank over every legacy-reported agent name --
-- most distinct active days, then most recent sync, then stable name (the
-- historical 044 ordering). The fallback restores an honest label without
-- inventing attribution: fallback rows keep top_agent_tokens = 0, and the
-- agent_breakdown plus its completeness flag still come from exact facts
-- only. Raw usage stays service-role only.
-- ============================================================
...
comment on function public.agent_token_leaderboard(
  ...
  'Service-only exact-numeric Burn Board aggregate. Exact token-weighted top agent wins; multi-agent legacy days fall back to a presence rank (distinct active days, then recency, then name) that restores the label with top_agent_tokens = 0 and never inflates the exact breakdown.';
